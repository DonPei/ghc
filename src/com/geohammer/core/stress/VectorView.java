...
public class VectorView extends TiledView {
	
	private float [] 	_qx1			= null; //ending point
	private float [] 	_qx2			= null;
	
	private double 		_arrowAngle		= 0.0;
	private double 		_arrowAngleCos	= 0.0;
	private double 		_arrowAngleSin	= 0.0;
	private int 		_arrowLengthInPixel = 10;
	...
	public double getArrowAngle() 					{ return _arrowAngle; }
	public int getArrowLengthInPixel() 				{ return _arrowLengthInPixel; }
	public float [] getQx1() 						{ return _qx1; }
	public float [] getQx2() 						{ return _qx2; }
	
	public void setArrowLengthInPixel(int arrowLengthInPixel) 	{ _arrowLengthInPixel = arrowLengthInPixel; }
	public void setarrowAngle(double arrowAngle) 				{ 
		_arrowAngle 		= arrowAngle; 
		_arrowAngleCos 		= Math.cos(arrowAngle*Math.PI/180.0);
		_arrowAngleSin 		= Math.sin(arrowAngle*Math.PI/180.0);
	}
	public void setQx1(float [] qx1) 				{ _qx1 = qx1; }
	public void setQx2(float [] qx2) 				{ _qx2 = qx2; }
	 ...
	 */
	public VectorView(float[] x1, float[] x2, float [] qx1, float [] qx2, int arrowLengthInPixel, double arrowAngle) {
		set(x1,x2);
		_qx1 = qx1;
		_qx2 = qx2;
		
		_arrowLengthInPixel = arrowLengthInPixel;

		_arrowAngle 		= arrowAngle;
		_arrowAngleCos 		= Math.cos(arrowAngle*Math.PI/180.0);
		_arrowAngleSin 		= Math.sin(arrowAngle*Math.PI/180.0);
	}
	public VectorView(float[][] x1, float[][] x2, int arrowLengthInPixel, double arrowAngle) {
		set(x1,x2);
		_qx1 = _x1.get(1);
		_qx2 = _x2.get(1);
	...
	public void paint(Graphics2D g2d) {		
		...
		Graphics2D gmarkEdge = (Graphics2D)g2d.create();
		gmarkEdge.setColor(Color.BLACK);
		gmarkEdge.setStroke(new BasicStroke(1));
		...
		int[] qx = new int[_nxmax];
		int[] qy = new int[_nxmax];
		...
		for (int is=0; is<1; ++is) {
			...
			computeXY(hp,vp,ts,n,qx1,qx2,qx,qy);
			
			// Draw lines between consecutive points.
			if (gline!=null) {
					//gline.drawPolyline(x,y,n);
					for(int i=0; i<n; i++) {
						gline.drawPolyline(new int []{x[i], qx[i]},new int []{y[i], qy[i]},2);
						//paintArrow(gline, 1, 20, 30, x[i], qx[i], y[i], qy[i]);
						//paintArrow(gline, -1, 20, 30, x[i], qx[i], y[i], qy[i]);
						paintArrow(gline, _arrowLengthInPixel, _arrowAngleSin, _arrowAngleCos, x[i], qx[i], y[i], qy[i]);
						paintArrow(gline, _arrowLengthInPixel, -_arrowAngleSin, _arrowAngleCos, x[i], qx[i], y[i], qy[i]);
					}
					
//				if(_showDensity) {
//					gline.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.5f));
//					 for (int ix=0; ix<n; ++ix)  {
//						 if(x2[ix]>0) gline.drawLine(x[ix],y0,x[ix],y[ix]); 
//					 }
//				} else if(_showRec) {
//					gline.setPaint(Color.red);
//					gline.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.3f));
//					gline.fillRect(x[0], y1, x[n-1]-x[0]+2, y0-y1);
//					//System.out.println("x0="+x[0]+" y0="+y0+" x1="+x[n-1]+" y1="+y1);
//				} else gline.drawPolyline(x,y,n);				
			}
			...
			if (gmark!=null) {
				if (_markStyle==Mark.POINT) {
					...
					paintFilledCircleWithEdge(gmark,gmarkEdge, markSize,n,x,y);
					...
					paintFilledSquareWithEdge(gmark,gmarkEdge, markSize,n,x,y);
					//paintFilledSquare(gmark,markSize,n,x,y);
				} else if (_markStyle==Mark.HOLLOW_SQUARE) {
					paintHollowSquare(gmark,markSize,n,x,y);
				} else if (_markStyle==Mark.STAR) {
					paintStar(gmark, markSize,x[0],y[0]);
				} else if (_markStyle==Mark.FILLED_STAR) {
					paintFilledStar(gmark, markSize,x[0],y[0]);
				}
				
			}

//			Graphics2D curve1 = (Graphics2D)g2d.create();
//			curve1.setFont(new Font ("Arial", Font.BOLD, 12));
//			
//			n = 2;
//			float[] ax1 = {_x1.get(0)[0], _x1.get(0)[0], _x1.get(0)[0]};
//			float[] ax2 = {0, 0, 0};
//			computeXY(hp,vp,ts,n,ax1,ax2,x,y);
//			
//			curve1.drawString("here", x[0], y[0]+5);
	...
	public ArrayList<Integer> getNx() { return _nx; }
	public ArrayList<float[]> getX1() { return _x1; }
	public ArrayList<float[]> getX2() { return _x2; }
	public ArrayList<float[]> getX3() { return _x3; }
	///////////////////////////////////////////////////////////////////////////
	// private
  ...
  private void paintLines(Graphics2D g2d, int n, int[] x, int[] y) {
	...
	private void paintStar(Graphics2D g2d, int innerRadius, int centerX, int centerY) {		
		int numRays = 5;
	...
	private void paintFilledStar(Graphics2D g2d, int innerRadius, int centerX, int centerY) {		
		int numRays = 5;
		double startAngleRad = Math.toRadians(-18);
		double outerRadius = innerRadius * 2.63;
		Path2D path = new Path2D.Double();
	...
	private void paintFilledCircleWithEdge(Graphics2D g2d, Graphics2D g2dEdge, int s, 
			int n, int[] x, int[] y) {
		int wh = 1+2*(s/2);
		int xy = wh/2;
		for (int i=0; i<n; ++i) {
			g2d.fillOval(x[i]-xy,y[i]-xy,wh,wh);
			g2dEdge.drawOval(x[i]-xy,y[i]-xy,wh-1,wh-1);
		}
	}
	private void paintArrow(Graphics2D g2d, int arrowLength, double arrowAngleSin, double arrowAngleCos, 
			int p1x, int p2x, int p1y, int p2y) 
	{
		int dpx = p1x-p2x;
		int dpy = p1y-p2y;
		double al = Math.sqrt(dpx*dpx+dpy*dpy);
		if(al<2) return;
		
		double ax = dpx*arrowAngleCos - dpy*arrowAngleSin;
		double ay = dpx*arrowAngleSin + dpy*arrowAngleCos;
		
		double ratio = arrowLength/al;
		double bx = ratio*ax;
		double by = ratio*ay;
		
		g2d.drawLine((int)(bx+p2x),(int)(by+p2y),p2x,p2y);
	}
	...
	private void paintFilledSquareWithEdge(Graphics2D g2d, Graphics2D g2dEdge, int s, 
			int n, int[] x, int[] y){
		int wh = 1+2*(s/2);
		int xy = wh/2;
		for (int i=0; i<n; ++i) {
			g2d.fillRect(x[i]-xy,y[i]-xy,wh,wh);
			g2dEdge.drawRect(x[i]-xy,y[i]-xy,wh-1,wh-1);
		}
	}
