...
public class PointsViewWithLabel extends TiledView {
	...
	public void setDataLabel(String [] dataLabel) 	{ _dataLabel = dataLabel; }
	 ...
	 */
	public PointsViewWithLabel(float[] x2) {
	 ...
	 * @param x2 array of x2 coordinates.
	 */
	public PointsViewWithLabel(Sampling s1, float[] x2) {
		set(s1,x2);
	}
	 ...
	 * @param x2 array of x2 coordinates.
	 */
	public PointsViewWithLabel(float[] x1, float[] x2) {
		set(x1,x2);
	}
	 ...
	 */
	public PointsViewWithLabel(float[] x1, float[] x2, float[] x3) {
		set(x1,x2,x3);
	}
	...
	public void paint(Graphics2D g2d) {
		...
		//if (_x3.size()>0) gtext = (Graphics2D)g2d.create();
		if (_dataLabel!=null) gtext = (Graphics2D)g2d.create();
		
		Graphics2D gLegend = null;
		if (_legendOn) {
			gLegend = (Graphics2D)g2d.create();
			gLegend.setFont(new Font ("Arial", Font.BOLD, 12));
		}
		...
		for (int is=0; is<_ns; ++is) {
			...
			if (gline!=null) {
				...
				if(_legendOn) {
					gline.drawPolyline(new int[]{_legendX, _legendX+_legendW}, 
							new int[]{_legendY-5, _legendY-5}, 2);
					gLegend.drawString(_legendLabel,_legendX+_legendW+10, _legendY);
				}
			}
			...
			if (gmark!=null) {
				if (_markStyle==Mark.POINT) {
					...
					if(_legendOn) paintPoint(gmark,1,new int[]{_legendX+_legendW/2}, new int[]{_legendY-5});
				} else if (_markStyle==Mark.PLUS) {
					paintPlus(gmark,markSize,n,x,y);
					if(_legendOn) paintPlus(gmark,markSize,1,new int[]{_legendX+_legendW/2}, new int[]{_legendY-5});
				} else if (_markStyle==Mark.CROSS) {
					paintCross(gmark,markSize,n,x,y);
					if(_legendOn) paintCross(gmark,markSize,1,new int[]{_legendX+_legendW/2}, new int[]{_legendY-5});
				} else if (_markStyle==Mark.FILLED_CIRCLE) {
					paintFilledCircle(gmark,markSize,n,x,y);
					if(_legendOn) paintFilledCircle(gmark,markSize,1,new int[]{_legendX+_legendW/2}, new int[]{_legendY-5});
				} else if (_markStyle==Mark.HOLLOW_CIRCLE) {
					paintHollowCircle(gmark,markSize,n,x,y);
					if(_legendOn) paintHollowCircle(gmark,markSize,1,new int[]{_legendX+_legendW/2}, new int[]{_legendY-5});
				} else if (_markStyle==Mark.FILLED_SQUARE) {
					paintFilledSquare(gmark,markSize,n,x,y);
					if(_legendOn) paintFilledSquare(gmark,markSize,1,new int[]{_legendX+_legendW/2}, new int[]{_legendY-5});
				} else if (_markStyle==Mark.HOLLOW_SQUARE) {
					paintHollowSquare(gmark,markSize,n,x,y);
					if(_legendOn) paintHollowSquare(gmark,markSize,1,new int[]{_legendX+_legendW/2}, new int[]{_legendY-5});
				}
				if(_legendOn) gLegend.drawString(_legendLabel,_legendX+_legendW, _legendY);
			}
...
//			}
			...
			if (gtext!=null) paintLabel(gtext,markSize,n,x,y,_dataLabel);
		}
	}
  ...
  private void paintLines(Graphics2D g2d, int n, int[] x, int[] y) {
	...
	private void paintLabel(
			Graphics2D g2d, int s, int n, int[] x, int[] y, String[] dataLabel) 
	{
		s /= 2;
		for (int i=0; i<n; ++i) {
			int xi = x[i];
			int yi = y[i];
			g2d.drawString(dataLabel[i],xi+s,yi-s);
		}
	}
}
