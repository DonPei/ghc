...
public abstract class CommonProject {	
	...
	public CommonProject(String projectFileName) {
		this(projectFileName, 20, 0);
	}
	public CommonProject(String projectFileName, int iId, int iUnit) {
		this.iId 		= iId;
		this.iUnit 		= iUnit;
		if(projectFileName!=null) setProjectName(projectFileName);
		setFileExtension(iId);
	}
	
	private void setFileExtension(int iId) {
		if(iId<10)  			extension 	= "prjres";
		
		else if(iId>=10 && iId<=19)  	extension 	= "prjvc";
		else if(iId>=20 && iId<=29)  	extension 	= "prj2d";
		else if(iId>=30 && iId<=39)  	extension 	= "prj3d";
		else   	extension 	= ".prjres";
	}
	
	public String getFullPathNoEndSeparator() 	{ return FilenameUtils.getFullPathNoEndSeparator(fullPath); }
	public String getProjectFileName() 			{ return getFullName()+"."+extension; }
	public String getFullName() 				{ return fullPath+baseName; }
	...
	public void setProjectName(String projectFileName) {
		...
		extension 	= FilenameUtils.getExtension(projectFileName);
	}	
	
	public String getResultsCwd() 				{ return createFolder(getFullPath()+getBaseName()+"_Results"); }
	public String getImagesCwd() 				{ return createFolder(getResultsCwd()+File.separator+"Images"); }
	public String getScratchCwd() 				{ return createFolder(getResultsCwd()+File.separator+"Scratch"); }
	...
	protected String walkingThroughFiles(int index, String srcPath, String extension) {
		...
		for (int i=0; i<listOfFiles.length; i++) {
			...
			if (listOfFiles[i].isFile()) {
				if(FilenameUtils.isExtension(name, extension) && k==index)  return srcPath+File.separator+name;
				k++;
			} else {
	...
	public void read() 		{ read(getProjectFileName()); }
	public void write() 	{ write(getProjectFileName()); }
		...
		try {
			...
			while( (line = reader.readLine()) != null ) {
				...
				if(k==0) { 
					...
					if(item.length>1) version = item[1].trim(); 
					else version = null; 
				}
				else if(k==1) { iUnit = Integer.parseInt(parseLine(line)); break;}
				k++;
			}
	...
	public void write(String inputFileName) {
		...
		try {
			...
			if(version!=null && version.isEmpty()) line += " "+version.trim();
...
//		String c = String.format("%d %d %s ", _iId, _iUnit, getProjectFileName()); 
//		return c;
//	}
