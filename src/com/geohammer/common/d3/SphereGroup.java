...
public class SphereGroup extends Group {
	 ...
	 */
	public SphereGroup(float[] xyz, float[] rgb) {
		buildTree(xyz,rgb);
	}
	 ...
	 * @param xyz array[3*np] of packed point coordinates.
	 */
	public SphereGroup(float size, float[] xyz) {
		this(size,xyz,null);
	}
	 ...
	 * @param rgb array[3*np] of packed color components.
	 */
	public SphereGroup(float size, float[] xyz, float[] rgb) {
		_size = size;
		buildTree(xyz,rgb);
	}
			...
			float[] xyz, float[] rgb) 
	{
		if (bbtNode.isLeaf()) {
			SphereNode pn;
			if (_size>0.0f) {
				pn = new SphereNode(bbtNode,_size,xyz,rgb);
			} else {
				pn = new SphereNode(bbtNode,xyz,rgb);
			}
			parent.addChild(pn);
		} else {
	...
	private class SphereNode extends Node {
		...
		protected void draw(DrawContext dc) {
			if(!SphereGroup.this.isVisible()) return;
