...
public class Podvin {
	...
	/*  April 1991, previous revision: 7 May 1993                                 */
	/*  This "robustified and ANSIfied" version is dated 12 July 2003             */
	/*                                                                            */
	/*  PROTOTYPE :                                                               */
	...
	/*      -DDEBUG_ARGS       (to check what function you called with what args) */
	/*                                                                            */
	/*  COMMON ERRORS:                                                            */
	/*       Incomprehensible behaviours in general result from C/Fortran         */
	...
	/*        of model boundaries, then array T will be left unchanged ! This is  */
	/*        now checked as an error condition)                                  */
	/*                                                                            */
	/*  LAST CHANGES ("robustification") :                                        */
	/*    This routine has be used intensively by many people in the past ten     */
	/*    years, but recently, I was informed of incomprehensible problems        */
	/*    encountered pseudo-randomly (and very rarely) by some users...          */
	/*    Here is what I found out and the solution I designed :                  */
	/*    Everyone should know (;=)) that no program should critically depend     */
	/*    on "exact" comparisons between floating point numbers, because these    */
	/*    numbers are NOT represented exactly on a computer. I only recently      */
	/*    became aware of the fact that this was a serious (and vicious) flaw     */
	/*    in this source code, that only reveals on some platforms (Intel-based   */
	/*    PCs in particular !) Technically, recursive calls in this program are   */
	/*    triggered by tests of the form (a<b) and, with no precaution, such      */
	/*    tests may be evaluated as true when a and b are evaluated with exactly  */
	/*    the same set of instructions yet in two different parts of the source   */
	/*    code. Clearly, a and b should then be exactly equal, but... they are    */
	/*    not and this results into infinite recursions crashing the program !    */
	/*    The problem is illustrated and explained as follows in gcc.info :       */
	...
	/*    In this "robustified" version, all critical tests (the ones triggering  */
	/*    recursive calls) have been "fuzzified" (i.e., a<b became b-a>eps) to    */
	/*    avoid this pathological behaviour. Because this depends on floating     */
	/*    point encoding, "eps" also is. It is in fact evaluated in accordance    */
	/*    with the imprecision of standard encoding schemes such as IEEE754.      */
	/*    Fuzzification is controlled by the macro parameter EPS_FUZZY.           */
	/*    If you get into trouble (i.e., encounter cases where time_3d crashes    */
	/*    after having entered an infinite loop of recursive calls), you might    */
	/*    consider changing this macro to a higher value, but PLEASE, be kind     */
	/*    enough to also inform me by e-mail, thanks!                             */
	/*                                                                            */
	...
	#define NINT(x)     (int)floor((x)+0.5)
	    /* NINT = "Nearest INTeger", used here only with positive numbers */
	#else
	#ifdef sun
	#ifdef BSD
	...
	static void
	    error(int),
	    init_nearest(void),
	    init_cell(float,float,float,int,int,int),
	    free_ptrs(int);
	...
	static float
	    timeshift;                        /* used to render all times >=0.0 */
	...
	#define NO_ERROR         0
	#define ERR_MULTINF      (-1)
	#define ERR_INF          (-2)
	#define ERR_RECURS       (-3)
	#define ERR_MALLOC       (-4)
	#define ERR_HS_EPS       (-5)
	#define ERR_NONPHYSICAL  (-6)
	#define ERR_MULTNONE     (-7)
	...
	#ifdef DEBUG_ARGS
	    fprintf(stderr,"Entering function time_3d using C-style interface\n");
	    fprintf(stderr,"Dimensions nx=%d ny=%d nz=%d\n",NX,NY,NZ);
	    fprintf(stderr,"(arrays stored in memory as C-array[nx][ny][nz])\n");
